...
FUNCTION izi, fluxin, errorin, idin, gridfile=gridfile, plot=plot, logOHsun=logOHsun, epsilon=epsilon, nz=nz, nq=nq, intergridfile=intergridfile, outgridfile=outgridfile, logzlimits=logzlimits, logqlimits=logqlimits, logzprior=logzprior, logqprior=logqprior, nonorm=nonorm, linear=linear, literature=literature
...
;     for i=0, nlines0-1 do fluxarr[i,*,*] = MIN_CURVE_SURF(grid0.flux[i], grid0.logz, grid0.logq, BOUNDS=[zarr[0], qarr[0], zarr[nz1-1], qarr[nq1-1]] , NX=nz1 , NY=nq1, /TPS) 
  ...
  d={id:id0, $                       ; line id
     ...
     ZM91:0., $                      ; Metallicity (McGaugh 1991 method)
     ZKK04:0., $                     ; Metallicity (Koboulnicky & Kewley 2004 method)
     qKK04:0., $       ; Ionization Parameter (Koboulnicky & Kewley 2004 method)
     ZKD02:0., $                      ; Metallicity (Kewley & Dopita 2002 method)
     ZZ94:0., $                      ; Metallicity (Kewley & Dopita 2002 method)
     ZPP04O3N2:0., $                      ; Metallicity (Kewley & Dopita 2002 method)
     ZPP04N2:0., $                      ; Metallicity (Kewley & Dopita 2002 method)
     ZP05:0., $                      ; Metallicity (Kewley & Dopita 2002 method)
     ZD02:0. $                      ; Metallicity (Kewley & Dopita 2002 method)
;     pdfjoint:fltarr(nz1, nq1), $    ; Joint PDF for log(Z) and log(q)
 ...
 ; stop
                                ...
                                ; CALCULATE LITERATURE DIAGNOSTICS
                                ; (ASUMING dust corrected fluxes)
  if (keyword_set(literature)) then begin

                                ; R23 diagnostics McGaugh 1991, Koboulnicky & Kewley 04
     if (d.flux[inoii3726] ne -666 and d.flux[inoiii5007] ne -666 and d.flux[inhbeta] ne -666 and d.flux[innii6584] ne -666) then begin
        n2o2=alog10(d.flux[innii6584]/d.flux[inoii3726])
        r23=alog10((d.flux[inoii3726]+d.flux[inoiii5007]/2.98+d.flux[inoiii5007])/d.flux[inhbeta]) ; asuming 5007/4959 ratio of 2.98 (Storey et al. 200)
        o32=alog10((d.flux[inoiii5007]/2.98+d.flux[inoiii5007])/d.flux[inoii3726])
        x=r23
        y=o32        

                                ; M91
        if (n2o2 le -1.2) then d.ZM91=12.-4.944+0.767*x+0.602*x^2-y*(0.29+0.332*x-0.331*x^2)
        if (n2o2 gt -1.2) then d.ZM91=12.-2.939-0.2*x-0.237*x^2-0.305*x^3-0.0283*x^4-y*(0.0047-0.0221*x-0.102*x^2-0.0817*x^3-0.00717*x^4)

                                ; KK04 recursive method
        o32=alog10(d.flux[inoiii5007]/d.flux[inoii3726])
        y=o32
        if (n2o2 le -1.2) then begin
           auxz0=8.2
           auxz1=auxz0
           sw=1
           cntr=0
           while (sw) do begin
              cntr=cntr+1
              auxz0=auxz1
              auxq=(32.81-1.153*y^2+auxz1*(-3.396-0.025*y+0.1444*y^2))/(4.603-0.3119*y-0.163*y^2+auxz1*(-0.48+0.0271*y+0.02037*y^2))
              auxz1=9.40+4.65*x-3.17*x^2-auxq*(0.272+0.547*x-0.513*x^2)
              print, auxz1, cntr
              if (cntr ge 3 and (abs(auxz1-auxz0) le 1e-3 or cntr ge 10)) then sw=0 
           endwhile
           d.zkk04=auxz1
           d.qkk04=auxq
        endif
        if (n2o2 gt -1.2) then begin
           auxz0=8.7
           auxz1=auxz0
           sw=1
           cntr=0
              ...
              auxz0=auxz1
              auxq=(32.81-1.153*y^2+auxz1*(-3.396-0.025*y+0.1444*y^2))/(4.603-0.3119*y-0.163*y^2+auxz1*(-0.48+0.0271*y+0.02037*y^2))
              auxz1=9.72-0.777*x-0.951*x^2-0.072*x^3-0.811*x^4-auxq*(0.0737-0.0719*x-0.141*x^2+0.0373*x^3-0.058*x^4)
;              print, auxz1, cntr
           ...
           d.qkk04=auxq
                                        ...
                                        ; KD02
        if (n2o2 gt -1.2) then begin
           auxcoef=[1106.866-n2o2, -532.15451, 96.373260, -7.8106123, 0.23928247] 
           auxroots=fz_roots(auxcoef)
           d.ZKD02=max(real_part(auxroots[where(abs(imaginary(auxroots)) le 0.1)]))
    
        endif
        
        if (n2o2 le -1.2) then d.ZKD02=mean([d.zm91, d.zkk04])
        
                                       ; Z94
        if (n2o2 gt -1.2) then begin

           d.ZZ94=9.265-0.33*x-0.202*x^2-0.207*x^3-0.333*x^4
    
        endif
    
        if (n2o2 le -1.2) then d.ZZ94=mean([d.zm91, d.zkk04])
        
                                ; PP04N2

        n2=alog10(d.flux[innii6584]/d.flux[inhbeta]/2.87)
        d.ZPP04N2=9.37+2.03*n2+1.26*n2^2+0.32*n2^3

                                ; PP04O3N3

        o3n2=alog10((d.flux[inoiii5007]/d.flux[inhbeta])/(d.flux[innii6584]/d.flux[inhbeta]/2.87))
        d.ZPP04O3N2=8.73-0.32*o3n2

                                ; P05
        
        r23=(d.flux[inoii3726]+d.flux[inoiii5007]/2.98+d.flux[inoiii5007])/d.flux[inhbeta]
        p=(d.flux[inoiii5007]/2.98+d.flux[inoiii5007])/d.flux[inhbeta]/r23

        if (n2o2 gt -1.2) then begin

           d.ZP05=(r23+726.1+842.2*p+337.5*p^2)/(85.96+82.76*p+43.98*p^2+1.793*r23)
    
        endif
    
        if (n2o2 le -1.2) then d.ZP05=(r23+106.4+106.8*p-3.40*p^2)/(17.72+6.60*p+6.95*p^2-0.302*r23)
